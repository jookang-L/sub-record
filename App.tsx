...
const App: React.FC = () => {
  ...
  // History Hook (Subject specific)
  const { history, addToHistory, updateHistoryItem, removeFromHistory } = useHistory('history_subject');
  const [currentHistoryId, setCurrentHistoryId] = useState<string | null>(null);

  // Debug: Log history on mount and changes
  useEffect(() => {
    console.log('[교과세특 App] 마운트됨, history 개수:', history.length);
    console.log('[교과세특 App] localStorage 확인:', localStorage.getItem('history_subject'));
  }, []);

  useEffect(() => {
    console.log('[교과세특 App] history 변경됨:', history.length, '개');
  }, [history]);
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      // Add to History
      const newId = addToHistory(generatedData, customSubjectName || `교과세특 - ${gradeLevel}`);
      setCurrentHistoryId(newId);
      console.log('[교과세특] 히스토리 추가됨:', newId, history.length);
    } catch (e: any) {
                ...
                {customInstructions && (
                  <button
                    onClick={() => {
                      ...
                      localStorage.removeItem('custom_instructions');
                    }}
