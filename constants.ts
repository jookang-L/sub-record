...
3. **글자 수 제한 엄수 (공백 포함)**: 각 등급별로 설정된 한계선을 절대 초과하지 마시오.
   - 1등급: 750자 이내
   - 2등급: 650자 이내
   - 3등급: 550자 이내
   *만약 내용이 길어지면 수식어를 삭제해서라도 무조건 이 제한을 맞추시오.
4. **섹션 헤더 포함 금지**: '탐구 동기', '탐구 과정', '탐구 결과', '평가 및 피드백' 등의 단어를 텍스트에 직접 포함하지 마시오. 내용은 이 흐름을 따르되, 소제목이나 구분자 없이 줄글로 자연스럽게 이어지도록 작성하시오.
...
[등급별 작성 기준]
- 1등급: 핵심 역량과 구체적 성과 위주의 고밀도 텍스트. (글자수: 750자 절대 엄수)
- 2등급: 활동 내용과 적절한 평가 포함. (글자수: 650자 절대 엄수)
- 3등급: 핵심 활동 위주, 요약된 평가. (글자수: 550자 절대 엄수)
...
{
  "standard": "표준 분량(약 700자) 버전의 텍스트",
  "gradeVersion": "사용자가 선택한 등급 기준 글자 수 제한을 완벽히 준수한 텍스트",
  "summary500": "500자 요약 버전",
  "summary300": "300자 요약 버전",
  "summary150": "150자 요약 버전"
}
`;

export const GRADE_DESCRIPTIONS: Record<GradeLevel, string> = {
  [GradeLevel.GRADE_1]: "매우 상세, 풍부한 평가 (750자 이내)",
  [GradeLevel.GRADE_2]: "적절히 구체적, 적절한 평가 (650자 이내)",
  [GradeLevel.GRADE_3]: "핵심 활동 위주, 요약된 평가 (550자 이내)",
};
